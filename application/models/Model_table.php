...
class Model_Table extends CI_Model{
    ...
    public function getTransactionSummary($date = null){
        $transactionSummary = $this->db->query("
            ...
            (
                select paid_purchase + paid_material_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + salary_payment
            ) as total_out,
            (
    ...
    public function employeeDue($employeeId = null, $date = null, $monthId = null) {
        $brunchId = $this->session->userdata('BRANCHid');
        
        $dueList = $this->db->query("
            select 
                emp.*,
                dep.Department_Name,
                des.Designation_Name,
                (
                    select ifnull(sum(shd.salary), 0)
                    from tbl_salary_sheet_details shd
                    join tbl_salary_sheet sh on sh.salary_sheet_id = shd.salary_sheet_id
                    where shd.employee_id = emp.Employee_SlNo
                    " . ($date == null ? "" : " and sh.process_date < '$date'") . "
                    " . ($monthId == null ? "" : " and sh.month_id = '$monthId'") . "
                ) as payable_amount,
                (
                    select ifnull(sum(ep.payment_amount), 0)
                    from tbl_employee_payment ep
                    where ep.Employee_SlNo = emp.Employee_SlNo
                    " . ($date == null ? "" : " and ep.payment_date < '$date'") . "
                    " . ($monthId == null ? "" : " and ep.month_id = '$monthId'") . "
                ) as paid_amount,
                (
                    select ifnull(sum(shd.deducted), 0)
                    from tbl_salary_sheet_details shd
                    join tbl_salary_sheet sh on sh.salary_sheet_id = shd.salary_sheet_id
                    where shd.employee_id = emp.Employee_SlNo
                    " . ($date == null ? "" : " and sh.process_date < '$date'") . "
                ...
                (
                    select payable_amount - (paid_amount + deducted_amount)
                ) as due
            from tbl_employee emp
            join tbl_department dep on dep.Department_SlNo = emp.Department_ID
            join tbl_designation des on des.Designation_SlNo = emp.Designation_ID
            where emp.status = 'a'
            and emp.Employee_brinchid = ?
            " . ($employeeId == null ? "" : " and emp.Employee_SlNo = '$employeeId'") . "
        ", $brunchId)->result();
