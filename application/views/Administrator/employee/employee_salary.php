	...
	<form @submit.prevent="savePayment">
		<div class="row" style="margin-top: 10px;margin-bottom:15px;border-bottom: 1px solid #ccc;padding-bottom: 15px;">
			...
			<div class="col-md-6">
				...
				<div class="form-group" style="display:none;" v-bind:style="{ display: paymentType == '' ? '' : 'none' }">
					<label class="control-label col-md-4">Payment Amount</label>
					<div class="col-md-7">
						<input type="text" class="form-control" v-model="payment.payment_amount">
					</div>
				</div>
				<div class="form-group" style="display:none;" v-bind:style="{ display: paymentType == 'deduct' ? '' : 'none' }">
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			getPayments() {
				axios.post('/get_employee_payments', { paymentType: this.paymentType }).then(res => {
					this.payments = res.data.map(p => {
						if(this.paymentType == '') {
							p.amount = p.payment_amount;
						} else if(this.paymentType == 'deduct') {
							p.amount = p.deduction_amount;
						}
