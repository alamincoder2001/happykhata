		...
		<div class="row" style="padding-top:10px;">
			...
			<div class="col-md-6">
				...
				<!-- <table class="table table-bordered table-condensed">
					<thead>
						<tr style="background: #dee4dc;">
							<th colspan="4">Salary Payments</th>
						</tr>
						<tr>
							<th>Employee Id</th>
							<th>Employee Name</th>
					...
					<tbody style="display:none;" v-bind:style="{display: salaryPayments.length > 0 ? '' : 'none'}">
						<tr v-for="payment in salaryPayments">
							<td>{{ payment.Employee_Id }}</td>
							<td>{{ payment.Employee_Name }}</td>
							<td>{{ payment.date }}</td>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="3" style="text-align:right">Total</td>
							<td style="text-align:right;">
								<span v-if="salaryPayments.length == 0">0.00</span>
								<span style="display:none;" v-bind:style="{display: salaryPayments.length > 0 ? '' : 'none'}">{{ totalSalaryPayments | decimal }}</span>
							</td>
						</tr>
					</tfoot>
				</table> -->
...
<script>
	new Vue({
		...
		computed: {
			...
			totalSalaryPayments(){
				return this.salaryPayments.reduce((prev, curr) => {
					return prev + parseFloat(curr.amount)
				}, 0).toFixed(2);
			},
			totalCashIn(){
			...
			totalCashOut(){
				return parseFloat(this.totalPurchase) +
					...
					parseFloat(this.totalSalaryPayments);
			},
			cashBalance(){
